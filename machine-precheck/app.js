...
const PRE_HOLD=500,FLASH_DURATION=1000,POST_HOLD=500,SPLASH_DURATION=PRE_HOLD+FLASH_DURATION+POST_HOLD;
...
function run(img,now){
  ...
  if(elapsed<PRE_HOLD){
    if(lastFrame!==-2){lastFrame=-2;ctx.fillStyle='#000';ctx.fillRect(0,0,canvas.width,canvas.height);}
  }else if(elapsed<PRE_HOLD+FLASH_DURATION){
    const flashElapsed=elapsed-PRE_HOLD;
    ...
    if(lastFrame!==-3){lastFrame=-3;ctx.fillStyle='#000';ctx.fillRect(0,0,canvas.width,canvas.height);}
  }
